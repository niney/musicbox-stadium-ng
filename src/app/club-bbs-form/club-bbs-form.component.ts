...
import {DEL_CLUB_BBS_URL, GET_CLUB_BBS_URL, SAVE_CLUB_BBS_URL} from "../app.constants";
...
@Component({
    selector: 'app-club-bbs-form',
    templateUrl: './club-bbs-form.component.html',
    styleUrls: ['./club-bbs-form.component.scss']
})
export class ClubBbsFormComponent implements OnInit {

    item: MbClubBbs = new MbClubBbs();
    ...
    async initItem() {
        ...
        if (params['id']) {
            this.httpClient.get<CCObjectResult<MbClubBbs>>(GET_CLUB_BBS_URL + "?id=" + params['id']).subscribe(response => {
                if (!response.result) {
                    return;
                }
    ...
    save(): void {
        ...
        this.httpClient.post<CCResult>(SAVE_CLUB_BBS_URL, this.item).subscribe(response => {
            if (!response.result) {
                alert('저장 실패');
                return;
            }
            this.router.navigate(['clubBbs', 'list']);
        });
    }
    ...
    del(): void {
        const params = this.activatedRoute.snapshot.params;
        if (params['id']) {
            this.httpClient.get<CCObjectResult<MbUser>>(DEL_CLUB_BBS_URL + "?id=" + params['id']).subscribe(response => {
                if (!response.result) {
                    alert('삭제 실패');
                    return;
                }
